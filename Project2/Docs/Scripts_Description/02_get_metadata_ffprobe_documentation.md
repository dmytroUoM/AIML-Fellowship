...
The script extracts **technical metadata** from a video file (default: `active.avi`) using `ffprobe.exe`, as the second step in a numbered pipeline (following the `01_` hashing script). It:
...
This script is the second step in the data-preparation pipeline for **Project 2: AIML-Driven Super-Resolution and Volumetric Reconstruction for Mixing Tanks**, an effort to apply AI/ML techniques to reconstruct volumetric detail from imaging data (e.g., electrical-impedance-style imaging of tank mixing behavior). Its business drivers include:
...
- **Reproducibility and dataset versioning**: Alongside the SHA256 hash generated in step 01, this JSON metadata record lets researchers later confirm not just *that* a video is unchanged, but *what exactly* it contained technically — supporting reproducible experiments and defensible research results.
...
- **Pipeline continuity**: The consistent `01_`, `02_`… naming and shared `Reports`/`Logs` folder structure indicate this script is designed to run as part of an ordered, auditable sequence (hash → metadata → further processing), likely followed by frame extraction, super-resolution inference, or volumetric reconstruction steps.

---

## Logging the Result

Because this script sits directly in the AI/ML data pipeline for the mixing-tank volumetric reconstruction project, its log and JSON report together form part of the **provenance and quality-control record** for each experimental video used in model training or inference.
...
- **Log levels used**: `INFO`, `SUCCESS`, `WARNING`, `ERROR` (more granular than the 01_ hashing script, which uses only `INFO`/`ERROR`)
- **Log content captured**:
  ...
  - Video file size and last-modified timestamp (a lightweight secondary integrity signal alongside the 01_ script's hash)
...
- **Combined value with 01_ script**: Together, the SHA256 hash (integrity) and this ffprobe JSON (technical characterization) give researchers a two-part fingerprint of each source video: proof it hasn't changed, and a precise record of what was actually captured — both important when training or validating AI/ML models sensitive to input data quality and consistency.
...
| Aspect | Assessment |
|---|---|
| **Complexity** | Low-to-moderate. More sophisticated than the 01_ script — includes parameters, a reusable logging function, JSON validation, and structured error capture from an external process. |
...
| **Maintainability** | High — parameterized design (`-VideoFileName`, `-NoLog`), a reusable `Write-Log` function, and clear inline documentation (including an ASCII folder-structure diagram in the header) make the script easy to understand, extend, and integrate into the broader numbered pipeline. |
...
- **Third-party binary dependency**: Unlike the 01_ hashing script, this script depends on an external executable (`ffprobe.exe`). Approvers should confirm the binary's source, version, and licensing (FFmpeg is LGPL/GPL depending on build configuration) are acceptable for the environment, and that it is deployed from a trusted, verified source.
...
- **Risk Level**: Low — read-only interaction with source data, no network activity, no credential handling; the only added risk factor versus the 01_ script is reliance on a third-party executable (`ffprobe.exe`), which should be sourced and verified through a controlled process.
...
- **Data Sensitivity**: `active.avi` is understood (from the broader project context) to contain scientific imaging data — captures related to volumetric/mixing-tank experiments intended for AI/ML super-resolution and reconstruction work — rather than generic video content. This carries the same research-data considerations as the companion hashing script:
